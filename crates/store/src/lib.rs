...
pub trait Store {
    ...
    async fn get(&self, key: &Slice) -> Result<Slice>;
    ...
    async fn put(&self, key: &Slice, value: &Slice) -> Result<()>;
    ...
    /// * `Err(_)` - If the operation failed
    async fn delete(&self, keys: &[Slice]) -> Result<()>;
    ...
    async fn scan(&self, prefix: &Slice) -> Result<KeyStream>;
}
